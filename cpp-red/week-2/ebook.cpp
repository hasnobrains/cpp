...
class ReadingManagerOld
{
  public:
    ReadingManagerOld()
        : user_page_counts_(MAX_USER_COUNT_ + 1, 0), sorted_users_(),
...
class ReadingManager
{
  public:
    void Read(int user_id, int page_count)
    {
        ...
        {
            int page_id = user_pages[user_id];
            page_users[page_id] -= 1;
            if (page_users[page_id] == 0)
            {
                page_users.erase(page_id);
            }
        }
    ...
    double Cheer(int user_id) const
    {
        ...
        const int user_count = user_pages.size();
        ...
        auto it = page_users.upper_bound(user_pages.at(user_id));

        --it;

        int sum =
            accumulate(page_users.begin(), it, 0,
